...
{   //태그 인식되면, 컨텐츠 로딩하는 스크립트
    ...
    void Update()
    {
        ...
        if (tmp != null && Application.platform == RuntimePlatform.Android)
        {
            //heading.text = "rea";
            //Debug.Log(tmp);
            scienscope_illust.transform.position = sc_illust_origin + new Vector3(0, (tmp.distance) * 0.00125f, 0);
            ...
            if (!(tmp.tag_id[0] == 0 && tmp.tag_id[1] == 0 && tmp.tag_id[2] == 0 && tmp.tag_id[3] == 0) && !scene_detected)
            {
                ...
                {
                    string rfid_tmp = RFID_address[i].Replace(":", "");
                    if (rfid_tmp.Equals(id))
                    {
                        scene_detected = true;
